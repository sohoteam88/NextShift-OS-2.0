...
case "$stage" in
  stage-1-3 | stage-4)
    [[ "$migration_rehearsal" == 'PASS' ]] || \
      fail "${stage} migration rehearsal evidence is not PASS"
    [[ "$migration_image_rehearsal" == 'PASS' ]] || \
      fail "${stage} migration image rehearsal evidence is not PASS"
    [[ "$migration_image_digest" =~ ^sha256:[0-9a-f]{64}$ ]] || \
      fail "${stage} migration image digest evidence is invalid"
    ;;
esac
