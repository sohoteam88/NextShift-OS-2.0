...
Every superadmin write records at least actor ID and role, action, target type and ID, target tenant when applicable, request/correlation ID, success or failure, redacted metadata, and timestamp. Tokens, secrets, passwords, and complete sensitive payloads are prohibited from audit metadata. Tenant-targeted writes use the target tenant's `tenantId`. Platform-global writes remain an unresolved architecture decision because the current Prisma `AuditLog.tenantId` is required: U3ADR must choose an independently reviewed option (nullable tenant plus scope, a separate `PlatformAuditLog`, or proof that no platform-global mutation exists) before U3B dispatch. No schema change is authorized here.
...
Architecture Review clarification: Steven's approval applies to the amendment direction; the governance contract remains pending exact-head Architecture Review until PR #96 receives PASS and is merged. Repository-wide discovery and the machine-readable `U3_AUDITLOG_ADR_GATE.json` add U3ADR between U3A and U3B. U3B remains Manifest `blocked` until that task records a fresh exact-head PASS decision and a separate governance adoption explicitly unblocks it. The mandatory new audit scope is every `/api/v1/superadmin/*` write; this amendment does not independently expand audit requirements for `/api/v1/admin/*` writes beyond existing approved behavior.
