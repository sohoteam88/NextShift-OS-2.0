...
import { ArrowLeft, ArrowUp, ArrowDown, Brain, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4 pb-12">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3"><button onClick={() => router.push('/dashboard')}><ArrowLeft className="h-5 w-5 text-gray-400" /></button><div><h1 className="text-xl font-bold">Analytics Intelligence Center</h1><p className="text-xs text-gray-500">Facts from analytics. Conclusions from canonical projections.</p></div></div>
        {ac && <div className={cn('rounded-full px-3 py-1.5 text-xs font-bold', ac.health.level==='high'?'bg-emerald-100 text-emerald-700':ac.health.level==='medium'?'bg-amber-100 text-amber-700':'bg-red-100 text-red-700')}>Business State: {ac.health.overallScore}%</div>}
      </div>
      ...
      {ac && (
        <>
          ...
          {/* Business State */}
          <S title="🏥 Readiness · Business State">
          ...
          {/* Projection Insights */}
          <S title="🧠 Projection Insights" className="border-purple-200 bg-gradient-to-br from-purple-50 to-white">
            {ac.insights.map(i => (
          ...
          {/* Journey State */}
          <S title="🎯 Progress · Journey State">
            {ac.actions.map((a,i) => (
          ...
          <S title="📈 Growth · Growth Loop">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 mt-1">{ac.benchmark.progress}% growth score</p>
              </div>
            </div>
