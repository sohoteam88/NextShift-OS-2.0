...
import { ArrowRight, Award, BarChart3, CheckCircle2, Circle, Compass, Trophy } from 'lucide-react';
...
type AchievementDef = { key: string; title: string; description: string; icon: string; xp: number };
type UnlockedAchievement = { key: string; title: string; description: string; icon: string; xpAwarded: number; unlockedAt: string };
...
function AchievementsTab({ totalXP }: { totalXP: number }) {
  ...
  const data = (achievements.data as { data?: { unlocked?: UnlockedAchievement[]; all?: AchievementDef[] } } | undefined)?.data;
  const unlocked: UnlockedAchievement[] = data?.unlocked ?? [];
  const all: AchievementDef[] = data?.all ?? [];
  const unlockedByKey = new Map(unlocked.map((item) => [item.key, item]));
  if (achievements.isLoading) return <Skeleton className="h-80 w-full rounded-[var(--radius-lg)]" />;
  return (
    <section className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-white p-5 shadow-sm">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div><h2 className="text-xl font-semibold text-[var(--color-text)]">我的成就</h2><p className="mt-1 text-sm text-[var(--color-text-muted)]">已解锁 {unlocked.length}/{all.length}</p></div>
        <div className="inline-flex items-center gap-2 rounded-full bg-amber-50 px-3 py-1.5 text-sm font-semibold text-amber-700"><Trophy className="h-4 w-4" />{totalXP} XP</div>
      </div>
      <div className="space-y-3">
        {all.map((def: any) => {
          const u = unlockedByKey.get(def.key);
          const isUnlocked = !!u;
          return (
            <div key={def.key} className={cn('rounded-[var(--radius-md)] border p-4', isUnlocked ? 'border-amber-200 bg-amber-50' : 'border-[var(--color-border)] bg-[var(--color-surface)] opacity-75')}>
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-sm font-semibold text-[var(--color-text)]">{isUnlocked ? 'Unlocked' : 'Locked'} · {def.title}</p>
                <span className="text-xs font-semibold text-[var(--color-text-muted)]">{isUnlocked && u ? `已解锁 · ${formatDate(u.unlockedAt)}` : '未解锁'}</span>
              </div>
              <p className="mt-1 text-sm text-[var(--color-text-muted)]">{def.description} · +{def.xp} XP</p>
            </div>
          );
        })}
      </div>
  ...
  return (
    <div className="space-y-4">
      {phases.map((phase, index) => {
        ...
        return (
          <section key={phase.title} className={cn('rounded-[var(--radius-lg)] border bg-white p-5 shadow-sm', current ? 'border-blue-200 ring-2 ring-blue-50' : 'border-[var(--color-border)]')}>
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="flex min-w-0 gap-3">
                <Icon className={cn('mt-0.5 h-5 w-5 shrink-0', complete ? 'text-emerald-600' : current ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]')} />
                <div>
                  ...
                  <p className="mt-2 text-xs font-semibold uppercase tracking-widest text-[var(--color-text-muted)]">完成标准：{phase.criteria}</p>
                </div>
              </div>
              {current ? <Link href={phase.steps[0].href} className="inline-flex h-10 items-center gap-2 rounded-[var(--radius-md)] bg-[var(--color-primary)] px-4 text-sm font-semibold text-white">开始下一步<ArrowRight className="h-4 w-4" /></Link> : null}
            </div>
            <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
              {phase.steps.map((step) => <Link key={step.href} href={step.href} className="flex h-11 items-center justify-between rounded-[var(--radius-md)] border border-[var(--color-border)] px-3 text-sm font-medium text-[var(--color-text)] hover:border-blue-200 hover:bg-blue-50">{step.label}<ArrowRight className="h-4 w-4 text-[var(--color-text-muted)]" /></Link>)}
            </div>
          </section>
        );
      })}
...
export default function JourneyPage() {
  ...
  // Derive beginner journey action from mission state
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold text-[var(--color-text)]">
            {advancedMode ? `${getFunnelLabel(funnelType, 'zh')} Journey` : '成长旅程'}
          </h1>
          <p className="mt-1 text-sm text-[var(--color-text-muted)]">
            {advancedMode ? '目标、瓶颈、下一步和成功路径。' : '一步一步，建立你的线上业务。'}
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={cn('inline-flex h-9 items-center gap-2 rounded-[var(--radius-md)] border px-3 text-xs font-medium transition-colors', advancedMode ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-[var(--color-border)] bg-white text-[var(--color-text-muted)] hover:bg-[var(--color-surface)]')}
          >
            <BarChart3 className="h-3.5 w-3.5" />
            {advancedMode ? '新手模式' : '高级模式'}
          </button>
        </div>
      </div>
      ...
      {advancedMode && (
        <>
          ...
          <div className="inline-flex rounded-[var(--radius-md)] border border-[var(--color-border)] bg-white p-1 shadow-sm">
            {[{ id: 'journey' as const, label: '旅程', icon: Compass }, { id: 'achievements' as const, label: '成就', icon: Award }].map((item) => {
              const Icon = item.icon;
              return <button key={item.id} type="button" onClick={() => setTab(item.id)} className={cn('inline-flex h-9 items-center gap-2 rounded-[var(--radius-sm)] px-3 text-sm font-semibold transition-colors', tab === item.id ? 'bg-blue-600 text-white' : 'text-[var(--color-text-muted)] hover:bg-[var(--color-surface)]')}><Icon className="h-4 w-4" />{item.label}</button>;
            })}
          </div>

          {mission.isLoading ? <Skeleton className="h-96 w-full rounded-[var(--radius-lg)]" /> : tab === 'journey' ? (
            <>
