...
Check: `tail -n 20 /home/deploy/backups/backup.log` and `ls -lt /home/deploy/backups/nextshift-*.dump.gz | head`.
...
```bash
pg_restore --clean --if-exists --no-owner --dbname "$ISOLATED_DATABASE_URL" /path/to/nextshift-YYYYMMDD-HHMMSS.dump.gz
```
...
```bash
scp deploy@<vps-host>:/home/deploy/backups/nextshift-YYYYMMDD-HHMMSS.dump.gz ./
```
