  ...
  return (
    <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-[var(--color-text)]">成长路线图</h3>
        <Link href="/journey" className="text-xs font-medium text-[var(--color-primary)] hover:underline flex items-center gap-1">查看完整地图 <ArrowRight className="h-3 w-3" /></Link>
      </div>
      <div className="flex items-center justify-between text-xs mb-2">
        <span className="text-[var(--color-text-muted)]">第 {roadmap.currentStep.stepNumber} 步 / 共 {roadmap.totalSteps} 步</span>
      ...
      <div className="flex items-center gap-1 flex-wrap">
        ...
        {lockedCount > 0 && <span className="text-[10px] text-gray-400 flex items-center gap-1"><Lock className="h-3 w-3" />+{lockedCount} 未解锁</span>}
      </div>
    </div>
  );
}
