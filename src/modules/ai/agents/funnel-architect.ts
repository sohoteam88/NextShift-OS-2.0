...
export async function executeFunnelArchitect(input: AgentExecutionInput): Promise<AgentExecutionReport> {
  ...
  if (!fb) { findings.push('还没有漏斗。'); recs.push('创建完整的转化漏斗，把引流磁铁和Webinar串起来。'); acts.push({ description: '创建漏斗', route: '/funnel', module: 'Funnel Builder' }); }
  if (lm && fb) { findings.push('漏斗基础已完成。'); recs.push('优化着陆页CTA和跟进序列。'); acts.push({ description: '优化漏斗转化', route: '/funnel', module: 'Funnel Builder' }); }
