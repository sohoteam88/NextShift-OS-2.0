...
export function FunnelBuilderDashboard() {
  ...
  const [type, setType] = React.useState<FunnelType>('lead_magnet');
  const [collapsed, setCollapsed] = React.useState(true);
  const health = pkg ? validateFunnelHealth(pkg) : null;
      ...
      {!pkg && (
        <div className="rounded-xl border-2 border-dashed border-blue-200 bg-blue-50 p-8 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-4">
            {(Object.entries(FUNNEL_TYPES) as [FunnelType, typeof FUNNEL_TYPES[FunnelType]][]).map(([k, v]) => (
