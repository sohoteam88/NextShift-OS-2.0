...
| F1 | ~~**P0:修复注册流程生产级断裂（悬空账号）**~~ **已完成（2026-07-14）**：PR #72（pipeline 第三圈产物 + 人工修复迭代）。根因:注册 API 在未验证会话下拒绝且无补偿路径。修复:tenant-provisioning-service（验证后自动补建租户）+ /setup-workspace 恢复页（悬空存量账号登录后引导补建）+ auth/callback 路由 + 三语"请先验证邮箱"提示文案 + E2E 恢复链路回归用例（dangling fixture 由 ensure-e2e-user 供给）。首轮 E2E 失败于恢复用例自身（provisioning 未 email_confirm + setup 页 hydration）,两个修复 commit 后 36 用例全绿。**生产验证待部署后执行**:+g2test 悬空账号登录恢复 + `user_signed_up` 面板见数（G2 遗留第 5 项） | 原始发现与断裂链见 [G2 核对报告](G2_POSTHOG_VERIFICATION_2026-07.md) 第 4 节 |
| G2 | ~~结果闸门 A 度量核对~~ **已完成（2026-07-14）**：4/5 事件面板实证可查;首要发现:instrumentation 此前为黑箱（host 空缺 + 幽灵 key）,当日修复（新 PostHog 项目 + PROD secrets + 重烤部署）;`user_signed_up` 不可验证——由此定位出 F1 的 P0 断裂。完整报告:[G2_POSTHOG_VERIFICATION_2026-07.md](G2_POSTHOG_VERIFICATION_2026-07.md) | 原要求:从生产 PostHog 后台拉取真实数据，确认 OS 3.6 M0 接的四个事件（`recommendation_viewed`/`recommendation_clicked`/`discussion_turn_sent`/`weekly_active`）以及既有的 `user_signed_up` 确实有真实数据在流入（不是"代码写了"，是"面板里能查到"），写入 blueprint。闸门本身（≥10 真实周活）达成与否不是本项验收标准，由 Steven 的获客轨决定 |
| G3 | ~~遗留 hygiene：`generateWithFallback` 收编进 router~~ **已完成（2026-07-15）**：PR #73。4 个遗留直连点全部收编进 router 并按语义分配 task category：`onboarding-service.ts` 两处 → `'brand_discovery'`（品牌定位，tier A）/ `'content_generation'`（首条内容，tier B），`voice-service.ts` 两处（主生成 + 严格 JSON 重试）→ `'brand_extraction'`（tier A），提示词／`temperature`／`maxTokens` 均未改动。voice 补上 `enforceQuota(user.tenantId)`（原本完全没有 AI 配额闸门，租户每日语音次数上限保持不变），重试分支仍聚合 token/duration 且改为记录**重试的** `routing`（即真正产出文本的模型）。`generateWithFallback` 已从 `providers/factory.ts` 删除、从 `modules/ai/index.ts` 取消导出，`grep -rn "generateWithFallback" src` 归零，通道无法回潮。新增 `onboarding-ai-router-callsite.test.ts` 与 `voice-ai-router-callsite.test.ts` 两个单测文件覆盖 category、配额顺序、routing 落日志与重试分支 | 原要求：Master Roadmap 记录为"2 处遗留"，实测（2026-07-13 grep）实际是 **4 处**：`src/modules/member/services/onboarding-service.ts`（326、381 行）和 `src/modules/voice/services/voice-service.ts`（303、316 行）直接调用 `providers/factory.ts` 的 `generateWithFallback`，绕过了 `src/modules/ai/router/ai-router.ts`。收编进 router，统一走 T1（OS 3.5）已经建立的路由+配额+日志路径 |
| F2 | ~~Twin 摘要去重与全字段有界（Round 审计 C-2 + A-2）~~ **已完成（2026-07-15）**：PR #74（commit `7cc1da7`）。(1) C-2 去重落在 **repository 层**（`interview-brand-business-twin-repository.ts`）:name 家族——`brand.brandName` 等于 `identity.businessName` 时省略 `brand.brandName`;positioning 家族——按优先级 `identity.positioning` > `identity.mission` > `brand.positioning`,与更高优先级字段文本相同者整键省略;比较按 `formatTwinField` 同款规范化（trim + 折叠内部空白）,保留原值仅规范化比对,去重后空 context 沿用既有 `Object.keys` 守卫省略;`buildTwinSummary` 仍完全不感知去重。(2) A-2:为 Business name / Industry / Business stage / Brand name / Brand voice 五个字段补 200 字符界,复用既有 `formatTwinField` 截断,无新常量/新策略。(3) 单测零断言删除、净增覆盖:repository 测试新增碰撞输入去重断言 + 全字段有界断言,原碰撞映射断言转为省略断言（强化而非删除）。全量门禁绿(type-check/test/build/lint),`packages/`、`prisma/`、`.env` 零 diff | 原要求:审计实证:BrandProfile 用户的 system prompt 中同一 positioning 文本以三个标签出现三次、business name 出现两次,且该块标注 "verified facts"——已是线上 prompt 噪音（上轮 F-5 预警未修即上线）。修复:(1) 在 **repository 层**（interview-brand-business-twin-repository）去重:mission/positioning/brand.positioning 相同文本仅保留一处,businessName/brandName 相同仅保留一处;buildTwinSummary 不感知去重逻辑;(2) A-2 顺手项:为其余 5 个未截断 Twin 字段补 maxLength 界（沿用 200 字符策略);(3) 单测:重复输入 → prompt 中每个事实仅出现一次 + 全字段有界断言。验收:既有 Twin 测试零断言删除（允许增强）,全量门禁绿。审计出处:[PIPELINE_AUDIT_20260715-000338](../../audit/PIPELINE_AUDIT_20260715-000338.md) |
...
5. Twin 摘要注入 system prompt，读取失败有 Sentry 可见降级;prompt 内容覆盖由**单测层持有**（逐属性断言:有界/空跳过/排序/降级——修订于 2026-07-15,理由:CI 无 LLM provider key,E2E 无法观察 prompt 内部;E2E 层持有恢复链路与响应契约覆盖。原文"E2E 有覆盖"经 Steven 批准修订,回应 Round 审计 C-1:发布标准的修改必须显式入档,不得由 commit 注释静默豁免）
