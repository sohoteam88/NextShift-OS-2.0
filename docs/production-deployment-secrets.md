...
Scope note: the four-link ancestry check is currently enforced only against the request PR. Applying `--merge` to the approval PR is preventive rather than currently validated — the approval artifact is an equally load-bearing trust anchor, and any future extension of the validator would otherwise be blocked by historical squash merges that can no longer be repaired. Do not infer from the absence of a present-day check that squashing the approval PR is safe.
