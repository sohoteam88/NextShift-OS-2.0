...
export async function getAICOODecision(userId: string, tenantId?: string): Promise<AICOODecision> {
  ...
  const [interviewAuthority, businessState, journeyState, missionAuthority, businessContext, growthLoopState, optimizationProjection, activationProjection, retentionProjection, valueProjection, expansionProjection, referralProjection] = await Promise.all([
