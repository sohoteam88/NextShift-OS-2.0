...
export async function executeCEOAdvisor(input: AgentExecutionInput): Promise<AgentExecutionReport> {
  ...
  if (ac.health.overallScore < 50) { f.push('业务基础还需加强。'); r.push('优先完成Brand DNA和Content Engine。'); a.push({ description: '完善品牌基础', route: '/brand-dna', module: 'Brand DNA' }); }
  else if (ac.health.overallScore < 80) { f.push('业务在增长中。'); r.push('重点优化漏斗转化和跟进系统。'); a.push({ description: '优化漏斗转化', route: '/funnel-builder', module: 'Funnel Builder' }); }
