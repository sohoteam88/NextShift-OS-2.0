...
export default function ContentPlanPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-semibold text-[var(--color-text)]">30天文案规划</h1>
        <p className="mt-1 text-sm text-[var(--color-text-muted)]">
          一键生成30天社媒内容日历——按 40% 教育 / 20% 故事 / 20% 权威 / 10% 推广 / 10% 社群的黄金比例规划，支持 Facebook、Instagram、TikTok、小红书四平台。
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {(Object.entries(TYPE_CONFIG) as [ContentType, typeof TYPE_CONFIG[ContentType]][]).map(([type, cfg]) => {
          ...
          return (
            <span key={type} className={cn('inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium', cfg.bg, cfg.color)}>
              {cfg.icon}{cfg.label} {pctMap[type]} ({countMap[type]}天)
            </span>
          );
        })}
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-white p-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-sm font-medium text-[var(--color-text)]">
              领域 / 行业 <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              value={form.niche}
              onChange={e => set('niche', e.target.value)}
              placeholder="例：副业系统 / 健康产品 / 英语教育 / 美容护肤"
          ...
          <div>
            ...
            <input
              ...
              placeholder="例：AI 副业课程 / 减肥代餐 / 一对一英语辅导"
              className="w-full rounded-[var(--radius-md)] border border-[var(--color-border)] px-3 py-2 text-sm outline-none focus:border-[var(--color-primary)] focus:ring-1 focus:ring-[var(--color-primary)]"
            />
          </div>
          <div className="sm:col-span-2">
            ...
            <input
              ...
              placeholder="例：马来西亚华人上班族25–38岁，想要多一份收入，不知道从哪里开始"
              className="w-full rounded-[var(--radius-md)] border border-[var(--color-border)] px-3 py-2 text-sm outline-none focus:border-[var(--color-primary)] focus:ring-1 focus:ring-[var(--color-primary)]"
            />
          </div>
          <div className="sm:col-span-2">
            <label className="mb-1 block text-sm font-medium text-[var(--color-text)]">个人故事 / 信任证明</label>
            <textarea
              value={form.personalStory}
              onChange={e => set('personalStory', e.target.value)}
              placeholder="例：我本身是全职妈妈，用了6个月通过副业额外赚了RM8,000……（不需要提具体金额，AI会自动处理）"
              rows={2}
              className="w-full rounded-[var(--radius-md)] border border-[var(--color-border)] px-3 py-2 text-sm outline-none focus:border-[var(--color-primary)] focus:ring-1 focus:ring-[var(--color-primary)]"
            />
          </div>
          ...
          <div>
            ...
            <select
              value={form.language}
              onChange={e => set('language', e.target.value as 'zh' | 'en' | 'ms')}
        ...
        <div className="mt-5 flex items-center justify-between">
          ...
          >
            {mutation.isPending ? (
              <><Loader2 className="h-4 w-4 animate-spin" />AI 规划中...</>
            ) : (
              <><Calendar className="h-4 w-4" />生成30天文案规划</>
            )}
          </button>
        </div>
      </form>
