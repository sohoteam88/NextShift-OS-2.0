...
| R-2 | **[MUST CONFIRM]** `.env.production` key audit: verify `retiredRevenueRuntimeFlag` and `NEXT_PUBLIC_ENABLE_RUNTIME_ANALYTICS` status. Absent → runtime path active (graduation intended). `'false'` → explicit legacy override. Either is valid — must be documented. | ⚠ Confirm |
