...
export async function executeContentDirector(input: AgentExecutionInput): Promise<AgentExecutionReport> {
  ...
  if (pillars.length === 0) { findings.push('尚未建立内容支柱。'); recommendations.push('先生成3-5个内容支柱作为创作骨架。'); actions.push({ description: '生成内容支柱', route: '/content-engine', module: '内容引擎' }); }
  if (publishedCount === 0) { findings.push('还没有发布任何内容。'); recommendations.push('先发布第一篇简单介绍贴，不要等完美。'); actions.push({ description: '发布第一篇内容', route: '/content-engine', module: '内容引擎' }); }
  if (publishedCount > 0 && publishedCount < 5) { findings.push('内容产出较低。'); recommendations.push('保持每周至少3篇内容的节奏。'); actions.push({ description: '生成30天内容日历', route: '/content-engine', module: '内容引擎' }); }
  if (pillars.length >= 3 && publishedCount >= 5) { findings.push('内容引擎运转良好。'); recommendations.push('可以开始视频内容了。'); actions.push({ description: '开始视频内容', route: '/video?view=production', module: '视频工作室' }); }
