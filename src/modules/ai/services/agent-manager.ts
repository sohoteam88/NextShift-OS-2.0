...
import type { AgentId, AgentExecutionInput, AgentExecutionReport, WorkforceState } from '../types/agents';
import { getAgent, getAgentsForPlan, getAgentsForMissionStage } from './agent-registry';

const AGENT_EXECUTORS: Record<AgentId, (input: AgentExecutionInput) => Promise<AgentExecutionReport>> = {
  brand_strategist: (i) => import('../agents/brand-strategist').then(m => m.executeBrandStrategist(i)),
  content_director: (i) => import('../agents/content-director').then(m => m.executeContentDirector(i)),
  video_producer: (i) => import('../agents/video-producer').then(m => m.executeVideoProducer(i)),
  funnel_architect: (i) => import('../agents/funnel-architect').then(m => m.executeFunnelArchitect(i)),
  traffic_strategist: (i) => import('../agents/traffic-strategist').then(m => m.executeTrafficStrategist(i)),
  sales_coach: (i) => import('../agents/sales-coach').then(m => m.executeSalesCoach(i)),
  crm_manager: (i) => import('../agents/crm-manager').then(m => m.executeCRMManager(i)),
  ceo_advisor: (i) => import('../agents/ceo-advisor').then(m => m.executeCEOAdvisor(i)),
};
