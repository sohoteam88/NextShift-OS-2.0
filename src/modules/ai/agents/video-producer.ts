...
export async function executeVideoProducer(input: AgentExecutionInput): Promise<AgentExecutionReport> {
  ...
  const vp = await import('@/modules/video-production/videoProductionService').then(m => m.videoProductionService.getLatestPackage(input.userId));

  if (!vp) { findings.push('还没有生成任何视频包。'); recs.push('选择一个内容支柱，生成第一支视频包。'); acts.push({ description: '生成第一支视频', route: '/video-production', module: 'Video Production' }); }
  else { findings.push(`已有视频包 (状态: ${vp.status})`); recs.push('继续生成更多视频内容，扩展你的视频库。'); acts.push({ description: '生成更多视频', route: '/video-production', module: 'Video Production' }); }
