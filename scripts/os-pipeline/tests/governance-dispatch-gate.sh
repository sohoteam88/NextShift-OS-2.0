...
policy_json() {
  jq -cn '{
    schema_version:1,gate_id:"U3-AUDITLOG-ADR",gate_task_id:"U3ADR",consumer_task_id:"U3B",policy_version:"2026-07-17.v2",
    ...
    required_decisions:["platform_global_storage","target_mapping","failure_audit_durability","tenant_deletion_audit_retention"],
...
write_decision() {
  ...
  jq -cn --arg option "$option" --arg pd "$policy_digest" --arg pp "$protected_digest" --arg proof "$proof_digest" '{
    ...
    policy_version:"2026-07-17.v2",policy_sha256:$pd,protected_paths_sha256:$pp,
    required_decisions:[
