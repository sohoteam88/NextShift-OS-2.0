# STD-006 Project Execution Orchestration Standard v1.1
...
Version 1.1 adds the mandatory repository audit resolution workflow for Conditional PASS outcomes.
...
Planning
  ...
  -> Verification
  -> Audit
  -> Audit Resolution, when required
  -> Release Preparation
  -> Production
  -> Maintenance
```
...
Conditional audit findings are corrected and re-audited before release preparation begins.
...
After final PASS audit evidence is available, generate:
...
- If findings remain, complete audit resolution and mandatory re-audit before Stop C.
- If release completes, continue to the next slice, next phase, production, or maintenance.

## Repository Audit Resolution

If Repository Audit returns PASS:
...
If Repository Audit returns CONDITIONAL PASS:
...
Release preparation must never begin directly after a Conditional PASS.

### Re-Audit Scope

The re-audit verifies only the reported findings and required correction evidence.

Typical checks:

- Required corrections applied
- Navigation updated, when navigation was part of the finding
- Repository consistency preserved
- `git diff --check`
- `git diff --cached --check`
- Scoped relative links resolve, when documentation links changed

If all reported findings are resolved, the audit result becomes PASS.

### Repository-First Re-Audit Rule

Re-audit must use repository artifacts only. Conversation history must not be used as evidence of correction.
...
7. If audit result is Conditional PASS, check correction evidence and final re-audit result.
8. Check release notes and release decision.
9. Check release package, deployment manifest, and alignment report when applicable.
10. Check release branch, tag, and production alignment when applicable.
...
- Audit complete with Conditional PASS means state is `Audit Resolution`.
- Audit resolution complete but final PASS re-audit missing means state remains `Audit Resolution`.
...
| Audit Resolution | Required Corrections, Re-Audit Result | STD-001 / STD-002 / STD-006 |
...
- A Conditional PASS audit must be resolved through required corrections and mandatory re-audit before Stop C.
- Release preparation must never begin directly from a Conditional PASS audit result.
- Re-audit evidence must come from repository artifacts, not conversation history.
