...
export async function executeCRMManager(input: AgentExecutionInput): Promise<AgentExecutionReport> {
  ...
  if (cc.hotLeads.length > 0) { f.push(`🔥 ${cc.hotLeads[0]?.name}需要立即跟进。`); r.push(cc.hotLeads[0]?.suggestedAction ?? '跟进Hot Lead'); a.push({ description: '跟进Hot Lead', route: '/crm', module: 'CRM' }); }
