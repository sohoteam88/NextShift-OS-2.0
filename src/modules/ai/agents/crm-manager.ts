...
export async function executeCRMManager(input: AgentExecutionInput): Promise<AgentExecutionReport> {
  ...
  f.push(`${cc.leads.total}位潜在客户，${cc.hotLeads.length}位需要优先跟进。`);
  if (cc.hotLeads.length > 0) { f.push(`🔥 ${cc.hotLeads[0]?.name}需要立即跟进。`); r.push(cc.hotLeads[0]?.suggestedAction ?? '跟进高意向潜在客户'); a.push({ description: '跟进高意向潜在客户', route: '/crm', module: 'CRM' }); }
  if (cc.followups.overdue > 0) { f.push(`${cc.followups.overdue}个跟进已逾期。`); r.push('今天完成逾期跟进。'); a.push({ description: '完成逾期跟进', route: '/crm', module: 'CRM' }); }
  if (cc.hotLeads.length === 0 && cc.leads.total > 0) { r.push('所有潜在客户都需要培养。继续发送有价值的内容。'); }
