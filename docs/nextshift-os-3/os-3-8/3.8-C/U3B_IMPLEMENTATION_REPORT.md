...
- Added `scripts/u3b-admin-migration/install-audit-idempotency-authority.sql` as the authoritative post-`db push` SQL bootstrap, and installed it explicitly in both GitHub Tests and E2E schema setup with `prisma db execute --schema prisma/schema.prisma`. The disposable PostgreSQL fixture executes that same CLI form, preserving the reviewed partial decision instead of replacing it with a full unique index.
