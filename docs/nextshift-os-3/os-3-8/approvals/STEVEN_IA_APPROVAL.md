...
Every superadmin write records at least actor ID and role, action, target type and ID, target tenant when applicable, request/correlation ID, success or failure, redacted metadata, and timestamp. Tokens, secrets, passwords, and complete sensitive payloads are prohibited from audit metadata. Tenant-targeted writes use the target tenant's `tenantId`. The U3ADR reviewed-decision proposal selects nullable `AuditLog.tenantId` plus an explicit tenant/platform scope, a database CHECK/backfill/index contract, durable PLATFORM-scoped tenant-deletion evidence, terminal fail-closed deleted-tenant enforcement across auth and workers, an append-only `AuditEventOutbox` for failure events after rollback, and database-enforced final-event idempotency with payload-conflict alerting. These are proposed implementation constraints under Steven's approved direction; they are not adopted until an exact-head PASS review and separate production-runner governance transaction. No schema change is authorized here.
