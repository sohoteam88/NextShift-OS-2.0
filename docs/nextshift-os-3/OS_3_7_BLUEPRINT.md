...
| F1 | **P0:修复注册流程生产级断裂（悬空账号）** | G2 核对时生产实测发现（见 [G2 核对报告](G2_POSTHOG_VERIFICATION_2026-07.md) 第 4 节）:注册提交报 "Authentication required" → Supabase 认证层用户已建、邮箱验证成功 → 但应用层 user/tenant 记录从未创建 → 用户永久悬空。**根因：**邮箱确认开启时 `signUp()` 不返回会话，随后的 register 请求因未认证被拒；确认链接原本直达 `/login`，没有回调/补偿路径在验证后将 Auth 用户与应用层 User/Tenant 对账。修复要求:(1) 诊断根因（注册 API 与邮箱验证的时序;为何 register 调用在未验证会话下被拒且验证后无补偿路径）;(2) 修复后完整链路可走通:注册→验证→登录→tenant 就绪→dashboard;(3) 为悬空存量账号提供恢复路径（登录时检测无 tenant → 引导补建,fixture: stevensc082+g2test@gmail.com）;(4) "Authentication required" 文案改为明确的"请先验证邮箱"指引;(5) E2E 补注册断裂回归用例（利用现有 provisioning 基建）;(6) 验收:`user_signed_up` 事件在 PostHog 面板可查（G2 遗留的第 5 项） |
