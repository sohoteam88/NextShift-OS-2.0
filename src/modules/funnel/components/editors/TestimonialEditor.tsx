...
type Props = { value: TestimonialSection; onChange: (v: TestimonialSection) => void };

export function TestimonialEditor({ value, onChange }: Props) {
  const setItem = (i: number, k: string, v: string) => {
    const items = [...value.items];
    items[i] = { ...items[i], [k]: v };
    onChange({ ...value, items });
  };
  const add = () => onChange({ ...value, items: [...value.items, { name: '', text: '' }] });
  ...
  return (
    <div className="space-y-3 py-3">
      ...
      <div>
        <p className="mb-1 text-xs font-medium text-gray-600">评价列表</p>
        <div className="space-y-3">
          {value.items.map((item, i) => (
            <div key={i} className="rounded border border-gray-200 p-2 space-y-2">
              <div className="flex items-center gap-2">
                <input className={inp} placeholder="姓名" value={item.name} onChange={e => setItem(i, 'name', e.target.value)} />
                <button onClick={() => remove(i)} className="text-red-400 shrink-0"><Trash2 className="h-4 w-4" /></button>
              </div>
              <textarea className={inp} rows={2} placeholder="评价内容" value={item.text} onChange={e => setItem(i, 'text', e.target.value)} />
            </div>
          ))}
          <button onClick={add} className="flex items-center gap-1 text-xs text-blue-600 hover:underline"><Plus className="h-3.5 w-3.5" />添加评价</button>
        </div>
      </div>
    </div>
  );
